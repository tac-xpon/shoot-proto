...
use piston_window::{ControllerButton, ControllerHat, HatState, Key};
...
fn main() {
    ...
    bg.0.set_cur_pos(0, 0)
        .put_achar_n(&AChar::new(' ', 1, BgSymmetry::Normal), VM_RECT_SIZE.0);
    ...
    'main_loop: loop {
        ...
        }
        {
            ...
            let vis = t_count % 2 == 0;
            spr.sp(1)
                .xy(my_pos.x + l_offset, my_pos.y + y_offset)
                .code(back_fire)
                .visible(vis);
            spr.sp(2)
                .xy(my_pos.x + r_offset, my_pos.y + y_offset)
                .code(back_fire)
                .visible(vis);
        }
        {
            ...
            const BURST_MAX: i32 = 8;
            const BURST_INTERVAL: i32 = 6;
            if input_role_state.get(InputRole::MainFire).0 {
                if burst_counter < BURST_MAX {
                    let (dx, dy) = {
                        //    let t = (t_count % 16) - 8;
                        //    if t < 0 { -t * 128 } else { t * 128 }
                        ((burst_counter % 8) * 200, -14 * 256 + 96)
                    };
                    let new_shot = Some((
                        (my_x256 + 21 * 256, my_y256 + 10 * 256),
                        (-dx, dy),
                        SHOT_PAT_NO,
                    ));
                ...
                burst_counter += 1;
                if burst_counter >= BURST_MAX + BURST_INTERVAL {
                    burst_counter = 0;
                }
            } else {
                burst_counter = 0;
        ...
        if input_role_state.get(InputRole::ViewRotLeft).1 & 0b1111 == 0b0011 {
            // Release -> Press
            game_window.turn_left();
        }
        if input_role_state.get(InputRole::ViewRotRight).1 & 0b1111 == 0b1100 {
            // Press -> Release
